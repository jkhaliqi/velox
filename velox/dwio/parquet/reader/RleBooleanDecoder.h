...
class RleBooleanDecoder : public facebook::velox::parquet::RleBpDecoder {
 public:
  ...
  RleBooleanDecoder(const char* start, const char* end, BufferPtr decompressedData_, const short* repetitionLevels_, int32_t& len)
  ...
  void readWithVisitor(const uint64_t* nulls, Visitor visitor) {
    ...
    for (;;) {
      if (hasNulls && allowNulls && bits::isBitNull(nulls, current)) {
        ...
        uint8_t* output = outputValues_.data();
        // decoder_->GetBatch(output, visitor.numRows());
        ...
        //   if (toSkip) {
        //     skip<hasNulls>(toSkip, i, nulls); // current instead of i?
        //     i += toSkip; //current +=?
        //   }
        // }
        // if (atEnd) {
        //   return;
        // }
        //toSkip = visitor.process(output[0] & 1, atEnd);
        if (!remainingValues_) {
          readHeader();
        }
        if (repeating_) {
